...
;
		...
		for (int i = 0; i < subCrystals.Length; i++)
		{
			...
			BounceUpAndDown(subCrystals[i], subCrystalMaxBounceHeight[i], subCrystalRotateSpeed[i]);
		}
	...
	void BounceUpAndDown(Transform obj, float maxHeight, float speed)
	{
		float y = MathFunctions.SmoothPingPong(timeElapsed, maxHeight, speed);
